 ...
 * 스텝이 둘 이상이 되면 subscribe/new처럼 useStepFlow를 도입한다.
 */
const OnboardingActivity: StaticActivityComponentType<"Onboarding"> = () => {
  // 진입하자마자 제안 이름이 채워져 있어야 하므로 lazy initializer로 한 번만 뽑는다.
  const [name, setName] = useState(() => generateRandomName());

  return (
    // 로그인 제공자 페이지에서 막 넘어온 화면이라 뒤로가기를 두지 않는다.
    // 뒤로 가면 앱이 아니라 인증 페이지로 되돌아간다. 스와이프백도 같은 이유로 막는다.
